...
class TransactionStorage:
    ...
    def _open_db(self, mode='c'):
        ...
        db_path = os.path.join(dir_path, 'transaction_storage.db')
    ...
    def _load_miners(self):
        with self._open_db() as db:
            if b'miners' in db:
                return set(json.loads(db[b'miners'].decode()))
            return set()

    def _save_miners(self):
        with self._open_db() as db:
            db[b'miners'] = json.dumps(list(self.miners)).encode()

    def get_transaction(self, hash):
        with self._open_db() as db:
            if hash in db:
                return Transaction.from_json(db[hash].decode())
        return None
    ...
    def add_transaction(self, transaction: Transaction, address_reward: str):
        ...
        with self._open_db() as db:
            db[transaction.txhash] = transaction.to_json().encode()
    ...
    def get_balance(self, address):
        with self._open_db() as db:
            ...
            return int(float(db.get(balance_key, b'0').decode()))

    def _update_balance(self, address, new_balance):
        with self._open_db() as db:
            balance_key = f"balance_{address}".encode()
            db[balance_key] = str(new_balance).encode()

    def get_nonce(self, address):
        with self._open_db() as db:
            nonce_key = f"nonce_{address}".encode()
            return int(db.get(nonce_key, b'0').decode())

    def _update_nonce(self, address, new_nonce):
        with self._open_db() as db:
            nonce_key = f"nonce_{address}".encode()
            db[nonce_key] = str(new_nonce).encode()
        ...
        with self._open_db() as db:
            for key in db.keys():
                if key.startswith(b'balance_'):
                    ...
                    balances[address] = int(float(db[key].decode()))
        return balances
    ...
    def to_json(self):
        with self._open_db() as db:
            transactions = {k.decode(): v.decode() for k, v in db.items() if not k.startswith(b'nonce_') and not k.startswith(b'balance_') and not k.startswith(b'miners')}
        return json.dumps({
            'balances': self.get_all_balances(),
            'nonces': {k: self.get_nonce(k) for k in self.nonces.keys()},
    ...
    def from_json(cls, json_str):
        ...
        with storage._open_db() as db:
            ...
            for address, balance in data['balances'].items():
                db[f"balance_{address}".encode()] = str(balance).encode()
            for address, nonce in data['nonces'].items():
                db[f"nonce_{address}".encode()] = str(nonce).encode()
        ...
        with self._open_db() as db:
            for key in db.keys():
                if not key.startswith(b'nonce_') and not key.startswith(b'balance_') and not key.startswith(b'miners'):
                    transaction = Transaction.from_json(db[key].decode())
